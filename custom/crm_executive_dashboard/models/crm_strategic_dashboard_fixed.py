...
class CRMStrategicDashboard(models.Model):
    ...
    def _get_financial_performance(self, date_from, date_to, team_ids):
        ...
        try:
            ...
            _logger.error(f"Error in _get_financial_performance: {str(e)}")
            return {}
    ...
    def _get_market_intelligence(self, date_from, date_to, team_ids):
        ...
        try:
            ...
            _logger.error(f"Error in _get_market_intelligence: {str(e)}")
            return {}
    ...
    def _get_customer_insights(self, date_from, date_to, team_ids):
        ...
        try:
            ...
            _logger.error(f"Error in _get_customer_insights: {str(e)}")
            return {}
    ...
    def _get_operational_efficiency(self, date_from, date_to, team_ids):
        ...
        try:
            ...
            _logger.error(f"Error in _get_operational_efficiency: {str(e)}")
            return {}
    ...
    def _get_risk_indicators(self, date_from, date_to, team_ids):
        ...
        try:
            ...
            _logger.error(f"Error in _get_risk_indicators: {str(e)}")
            return {}
    ...
    def _get_predictive_analytics(self, date_from, date_to, team_ids):
        ...
        try:
            ...
            _logger.error(f"Error in _get_predictive_analytics: {str(e)}")
            return {}
    ...
    def _get_team_performance_overview(self, date_from, date_to, team_ids):
        ...
        try:
            ...
            _logger.error(f"Error in _get_team_performance_overview: {str(e)}")
            return []

    @api.model
    def generate_strategic_report(self, report_type='excel', date_from=None, date_to=None, team_ids=None):
        """Generate strategic report with proper error handling"""
        try:
            data = self.get_strategic_dashboard_data(date_from, date_to, team_ids)
            
            if 'error' in data:
                raise UserError(_("Failed to retrieve dashboard data: %s") % data['error'])
            
            if report_type == 'excel':
                return self._generate_excel_report(data, date_from, date_to)
            else:
                return self._generate_csv_report(data, date_from, date_to)
                
        except Exception as e:
            _logger.error(f"Error generating strategic report: {str(e)}")
            raise UserError(_("Failed to generate report: %s") % str(e))

    def _generate_excel_report(self, data, date_from, date_to):
        """Generate Excel report using safe methods"""
        try:
            ...
            try:
                ...
                worksheet = workbook.add_worksheet('Strategic Summary')
                
                # Add basic data
                worksheet.write(0, 0, 'Strategic CRM Dashboard Report')
                worksheet.write(1, 0, f'Period: {date_from} to {date_to}')
                worksheet.write(2, 0, f'Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
                
                # Add KPIs
                strategic_kpis = data.get('strategic_kpis', {})
                row = 4
                worksheet.write(row, 0, 'Strategic KPIs')
                row += 1
                
                for key, value in strategic_kpis.items():
                ...
                _logger.warning("xlsxwriter not available, generating CSV report instead")
                return self._generate_csv_report(data, date_from, date_to)
    ...
    def _generate_csv_report(self, data, date_from, date_to):
        ...
        try:
            ...
            if team_overview:
                writer.writerow(['Team Name', 'Revenue', 'Opportunities', 'Win Rate', 'Team Size'])
            ...
            _logger.error(f"Error generating CSV report: {str(e)}")
            raise UserError(_("Failed to generate CSV report: %s") % str(e))
