...
export class AccountDashboard extends Component {
  ...
  async FetchData() {
    ...
    this.state.income_chart = await this.orm.call(
      "account.move",
      "get_income_chart",
      [this.state.income]
    );
    this.state.payment_data = await this.orm.call(
      "account.move",
      "get_payment_data",
      [this.state.payment_list_filter, this.state.payment_data_filter]
    );
    this.state.top = await this.orm.call("account.move", "get_top_datas", [
      this.state.top_filter,
    ]);
    this.state.aged_payable = await this.orm.call(
      "account.move",
      "get_aged_payable",
      [this.state.aged_filter, this.state.aged_payable_filter]
    );
    this.state.top_sale_cust = await this.orm.call(
      "account.move",
      "get_sale_revenue",
      [this.state.top_sale_cust_filter]
    );
    this.state.balance = await this.orm.call(
      "account.move",
      "get_bank_balance",
      []
    );
  ...
  RenderChart() {
    this.aged_chart(
      this.AgedRecords.el,
      "bar",
      this.state.aged_payable.partner,
      "Amount",
      this.state.aged_payable.amount
    );
