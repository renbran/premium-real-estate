...
export class PaymentWorkflowController {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.notification.add(
          result.message || _t("Failed to submit payment"),
          {
            type: "warning",
          }
        );
  ...
  async approvePayment(paymentId, comments = "") {
    try {
      ...
      if (result.success) {
        ...
        this.notification.add(
          result.message || _t("Failed to approve payment"),
          {
            type: "warning",
          }
        );
  ...
  async rejectPayment(paymentId, reason) {
    try {
      ...
      if (result.success) {
        ...
        this.notification.add(
          result.message || _t("Failed to reject payment"),
          {
            type: "warning",
          }
        );
  ...
  async getApprovalHistory(paymentId) {
    try {
      return await this.orm.call("account.payment", "get_approval_history", [
        paymentId,
      ]);
    } catch (error) {
...
registry
  .category("fields")
  .add("payment_approval_widget", PaymentApprovalWidget);
...
console.log(
  "[PaymentWorkflow] Modern Odoo 17 payment workflow loaded successfully"
);
