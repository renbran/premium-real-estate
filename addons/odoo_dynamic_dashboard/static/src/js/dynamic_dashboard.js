...
export class OdooDynamicDashboard extends Component {
  ...
  onChangeTheme() {
    ...
    if (container) {
      container.setAttribute(
        "style",
        this.ThemeSelector.el.value + "min-height:-webkit-fill-available;"
      );
    }
  }
  ...
  async renderDashboard() {
    ...
    try {
      const themeResponse = await this.orm.call(
        "dashboard.theme",
        "get_records",
        [[]]
      );
      themeResponse.forEach((ev) => {
        const options = document.createElement("option");
        options.value = ev.style;
        options.text = ev.name;
        this.ThemeSelector.el.append(options);
      });

      const dashboardResponse = await this.orm.call(
        "dashboard.block",
        "get_dashboard_vals",
        [[], this.actionId]
      );
      for (let i = 0; i < dashboardResponse.length; i++) {
        ...
        if (dashboardResponse[i].type === "tile") {
          mount(DynamicDashboardTile, itemsContainer, {
            props: {
              widget: dashboardResponse[i],
          ...
          mount(DynamicDashboardChart, itemsContainer, {
            props: {
              widget: dashboardResponse[i],
              doAction: this.action,
              rpc: this.rpc,
  ...
  changeViewMode(ev) {
    ...
    if (currentMode == "light") {
      ...
      $(".container").attr(
        "style",
        "background-color: #383E45;min-height:-webkit-fill-available; !important"
      );
      $(".mode").attr("mode", "dark");
      $(".bi-moon-stars-fill").attr(
        "class",
        "bi bi-cloud-sun-fill view-mode-icon"
      );
      $(".bi-cloud-sun-fill").attr("style", "color:black;margin-left:10px;");
      $(".mode").attr("style", "display: none !important");
      $("#search-input-chart").attr(
        "style",
      ...
      $("#dropdownMenuButton").attr(
        "style",
        "background-color: #03DAC5;margin-top:-4px; !important"
      );
      $("#text_add").attr("style", "color: black; !important");
      $(".date-label").attr(
        "style",
        "color: black;font-family:monospace; !important"
      );
      ...
      $(".bi-cloud-sun-fill").attr(
        "class",
        "bi bi-moon-stars-fill view-mode-icon"
      );
      $(".view-mode-icon").attr(
        "style",
        "color:black;margin-left:10px; !important"
      );
      $(".mode").attr("style", "display: none !important");
      ...
      $("#dropdownMenuButton").attr(
        "style",
        "background-color: none;margin-top:-4px; !important"
      );
      $("#text_add").attr("style", "color: white; !important");
      $(".date-label").attr(
        "style",
        "color: black; !important;font-family:monospace; !important"
      );
  ...
  onClickAddItem(event) {
    ...
    let chart_type;
    if (type == "graph") {
      chart_type = event.target.getAttribute("data-chart_type");
    }
    if (type == "tile") {
      const randomColor =
        "#" +
  ...
  async dateFilter() {
    ...
    try {
      const response = await this.orm.call(
        "dashboard.block",
        "get_dashboard_vals",
        [[], this.actionId, start_date, end_date]
      );
  ...
  async clickSearch() {
    ...
    const input = document.querySelector("#search-input-chart")?.value;

    try {
      const response = await this.rpc("/custom_dashboard/search_input_chart", {
        search_input: input,
      });
      const blocks = document.querySelectorAll(".items .resize-drag");
      blocks.forEach((element) => {
        const dataId = element.dataset.id;
        if (response.includes(parseInt(dataId))) {
  ...
  clearSearch() {
    ...
    const searchInput = document.querySelector("#search-input-chart");
  ...
  async printPdf() {
    ...
    const elements = document.querySelectorAll(".items .resize-drag");
    const newElement = document.createElement("div");
    newElement.className = "pdf";

    elements.forEach((elem) => {
      newElement.appendChild(elem.cloneNode(true));
    });

    for (let x = 0; x < newElement.children.length; x++) {
    ...
    try {
      await html2pdf().set(opt).from(newElement).save();
      window.location.reload();
    } catch (error) {
